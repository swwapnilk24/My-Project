...
class Organisation extends React.Component {
  constructor(props) {
    ...
    this.onLtiChange = this.onLtiChange.bind(this);
    this.rangeSliderVal = 0;
    this.state = { myModel: null, myDiagram: null, data: rawData, currentLti: props.lti_range, volume: props.lti_range };
  }

  componentWillMount() {
    this.props.dispatch(recalculateBudget(this.state.volume, 'lti'));
  ...
  onLtiChange(value) {
    this.setState({ currentLti: value });
    this.props.dispatch({ type: OrgChart.CALCULATE_BUDGET, lti: value });
  }
  getResourceById(id) {
    let name = '';
  ...
  setViewType(viewType) {
    if (viewType === 'chart') {
      ...
      document.getElementById('ltiSlider').style.backgroundColor = 'White';
    } else {
      document.getElementById('chartview').style.display = 'none';
      document.getElementById('tableview').style.display = 'block';
      document.getElementById('ltiSlider').style.backgroundColor = 'transparent';
    }
  }
  reloadCanvas() {
  ...
  handleLTIChange = silderValue => {
    this.setState({
      volume: silderValue
    });
  };
  handleLTIChangeComplete = () => {
    this.props.dispatch({ type: OrgChart.CALCULATE_BUDGET, lti: this.state.volume });
  };
  ...
  render() {
    ...
    TableView = <div id="tableview"><DataGrid expanded={this.props.lti_table_expanded} current_view={'lti'} hierarchy={this.props.orgHierarchy} columns={gridOptions} data={this.state.data} /></div>;
    const LtiOptions = [
      {
        displayName: 'Min',
        value: 'min'
        ...
        <div className="row">
          <div className="col-xs-12">
            <div className="slider" id="ltiSlider">
              <MultiToggle
                options={LtiOptions}
                selectedOption={this.state.currentLti}
                onSelectOption={this.onLtiChange}
                label="LTI Select Plan Range"
              />
            </div>
            {ChartView}
            {TableView}
...
function MapStateToProps(state) {
  return {
    ...
    lti_range: state.OrgchartReducer.lti_range,
    orgHierarchy: state.OrgchartReducer.org_hierarchy,
    lti_table_expanded: state.OrgchartReducer.lti_table_expanded
  };
}
