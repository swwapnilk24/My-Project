...
class Organisation extends React.Component {
  constructor(props) {
    ...
    this.onIpfChange = this.onIpfChange.bind(this);
    this.rangeSliderVal = 0;
    this.state = { myModel: null, myDiagram: null, data: rawData, currentIPF: props.ipf_range, volume: props.ipf_range, selectedOption: 'master', meritVolume: 80, ltiVolume: 90 };
  }

  componentWillMount() {
    this.props.dispatch(recalculateBudget(this.state.volume, 'sti'));
  ...
  onIpfChange(value) {
    this.setState({ currentIPF: value });
    this.props.dispatch({ type: OrgChart.CALCULATE_BUDGET, sti: value });
  }
  getResourceById(id) {
    let name = '';
  ...
  setViewType(viewType) {
    if (viewType === 'chart') {
      ...
      document.getElementById('stiSlider').style.backgroundColor = 'White';
    } else {
      document.getElementById('chartview').style.display = 'none';
      document.getElementById('tableview').style.display = 'block';
      document.getElementById('stiSlider').style.backgroundColor = 'transparent';
    }
  }
  reloadCanvas() {
        ...
        goObj(go.Panel, 'Horizontal',
          ...
          goObj(go.Panel, 'Table',
            ...
            goObj(go.TextBlock, 'ABS ', textStyle,
              { row: 3, column: 0, width: 40 }),
            goObj(go.TextBlock, textStyle,
              { row: 4, column: 0, width: 40, text: 'row 2\ncol 0' },
              new go.Binding('text', 'ABS').makeTwoWay()),
            goObj(go.TextBlock, 'STI', textStyle,
                { row: 3, column: 1, width: 40 }),
            goObj(go.TextBlock, textStyle,
                { row: 4, column: 1, width: 40, text: 'row 2\ncol 1' },
              new go.Binding('text', 'STI').makeTwoWay()),
              goObj(go.TextBlock, 'Rating', textStyle,
              { row: 3, column: 4, width: 40 }),
            goObj(go.TextBlock, textStyle,
                { row: 4, column: 4, width: 40, text: 'row 2\ncol 3' },
              new go.Binding('text', 'rating').makeTwoWay()),
            goObj(go.TextBlock, 'Budget', textStyle,
              { row: 3, column: 3, width: 40 }),
            goObj(go.TextBlock, textStyle,
                { row: 4, column: 3, width: 40, text: 'row 2\ncol 3' },
              new go.Binding('text', 'StiBudget').makeTwoWay()),
              // test
              goObj(go.TextBlock, '-----------------------------', textStyle,
              { row: 5, column: 0, columnSpan: 7 }),
            ...
            goObj(go.TextBlock, 'TGT', textStyle,
              { row: 6, column: 4, width: 50 }),
            goObj(go.TextBlock, textStyle,
                { row: 7, column: 4, width: 50, text: 'row 4\ncol 3' },
              new go.Binding('text', 'TGT').makeTwoWay())
          )  // end Table Panel
  ...
  render() {
    ...
    TableView = <div id="tableview"><DataGrid expanded={this.props.sti_table_expanded} current_view={'sti'} hierarchy={this.props.orgHierarchy} columns={gridOptions} data={this.state.data} /></div>;
    const IPFOptions = [
      {
        displayName: 'Min',
        value: 'min'
      ...
      <div className="container">
        <div className="row">
          <div className="col-xs-12">
            <div className="slider" id="stiSlider">
              <MultiToggle
                options={IPFOptions}
                selectedOption={this.state.currentIPF}
                onSelectOption={this.onIpfChange}
                label="IPF Range"
              />
            </div>
            {ChartView}
...
function MapStateToProps(state) {
  return {
    ...
    ipf_range: state.OrgchartReducer.ipf_range,
    orgHierarchy: state.OrgchartReducer.org_hierarchy,
    sti_table_expanded: state.OrgchartReducer.sti_table_expanded
  };
}
